...
export default function AwardedQuotationsVendor() {
  ...
  const getData = async () => {
    await axios
      ...
      .then(({ data }) => {
        console.log(data);
        const filteredData = data.filter((e) => e.isQuotationAccepted && e.vendorDetails._id.toString() === sessionUser._id.toString());
      ...
      renderCell: (params) => {
        return (
          <div className="programsBox-button-div">
            ...
            {(params.row.approvalStatus === "pending" || params.row.approvalStatus === "rejected") && (
              <span
                style={{ border: "1px solid #d6193f", padding: "5px", borderRadius: "10px", cursor: "pointer" }}
            ...
            {params.row.isQuotationAccepted && !params.row.isSpocDetailsSubmitted && (
              <span
                style={{ border: "1px solid #d6193f", padding: "5px", borderRadius: "10px", cursor: "pointer" }}
                onClick={() => handleOpenSubmitSpoc(params.row._id)}
              >
                Submit SPOC
              </span>
            )}
